...
- [x] (v1.5) bUnit을 활용한 Blazor UI 컴포넌트 단위 테스트 구현
- [x] (v1.5) Playwright for .NET 기반 로그인, 저장소 관리, PR 및 이슈 트래커 핵심 시나리오 E2E 테스트 자동화
- [x] (v1.5) SSH 서버의 현대적 호환성 개선 (Microsoft.DevTunnels.Ssh 교체)

### Active

<!-- Current scope. Building toward these. -->

- [ ] (v1.6) 다음 마일스톤 요구사항 정의 예정

### Current State

**Shipped v1.5**: bUnit 및 Playwright for .NET 기반으로 단위/E2E UI 테스트 자동화를 전면 구축하였고, SSH 서버 라이브러리를 Microsoft.DevTunnels.Ssh로 전면 교체하여 ed25519 및 rsa-sha2를 비롯한 현대적 알고리즘 호환성을 달성하였습니다. 또한 SSH 접속 이력을 DB에 영구 기록하는 SshAuthLog 감사 로깅 기능도 추가하였습니다.
**Active v1.6**: 다음 개발 마일스톤에 진입하기 위한 계획 단계에 있습니다.

<details>
<summary>이전 릴리즈 내역 (Shipped v1.4, v1.3, v1.2, v1.1, v1.0)</summary>
...
| Microsoft.DevTunnels.Ssh 도입 (D-11) | FxSsh의 현대적 SSH 알고리즘(ed25519, rsa-sha2) 지원 부재 해결 | ✓ Good |
| SshAuthLog 데이터 로깅 (D-12) | 비등록 키 접속 차단 모니터링 및 실시간 침입 감사 지원 | ✓ Good |
| UI/E2E 테스트 이중화 (D-13) | 컴포넌트 렌더링(bUnit)과 전 시나리오 브라우저 E2E(Playwright) 종합 검증 | ✓ Good |
...
---
*Last updated: 2026-06-11 after milestone v1.5 complete*
